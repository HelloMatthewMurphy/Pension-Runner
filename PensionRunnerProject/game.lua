...
function scene:create(event)
    ...
    function updateHUD()
        if isRoundOver == false then
            ...
            local scoreText = display.newText( "", display.contentCenterX, 25, native.systemFontBold, 15 )
            group:insert(scoreText)
            local score = 999999999
                local newScore = 0
         
            local function lerp( v0, v1, t )
                return v0 + t * (v1 - v0) + (carSpeed*10)
            end
